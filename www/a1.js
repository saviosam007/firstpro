...
a1.start = function(hookElementSelection, dataurlForJsonFile) {
	...
	a1.bakedRecipes = [];
	//var load= '.loading';
	//var load=$("<div class='loading'>LOADING....</div>");
	...
	$.ajax({url:dataurlForJsonFile}).success(function(data) {
		...
		var logo = $("<div class='logo'><h5>Cake baby</h5></div>");
		logo.append('<img src="cakebaby1.jpg"/>');
		hookElementSelection.append(logo);
		logo.hide().delay(1000).slideDown(1000);
		console.log("savio");
...
var renderCalculator = function(base) {
	var costHTML = $("<br><br><br><br><br><br><br><br><button id='btn' class='button'>Calculate Weekly Total</button>");
	var resultsHTML = $("<div id='results' style='display: none;'></div>");
	costHTML.hide().delay(1000).slideDown(1000);
	...
	$(".recipe").delay('fast').slideUp(800).animate('fast',function(){
	$("#btn").slideUp(800).animate('slow', function(){$("#results").slideDown(800);
	});
	});
	//resultsHTML.delay('slow').hide('slow').slideUp('slow');		

	});
	base.append(costHTML);
	base.append(resultsHTML);
	//resultsHTML.delay('fast').hide('fast').slideUp('slow');
...
var splitTotalsAndPrint = function(base, ingredientTotals)
{
	...
	for (supplierName in supplierTotals) {
		...
		var ingredientHTML = $("<div></div><br><br><br><br><br><br><br><br><br><br><br>");
		...
		var supplierTotalHTML = $("<h3><span> Supplier Cost: $" + supplierTotalCost.toFixed(2) + "</span></h3>");
...
a1.Product = function(name,cost,per,supplier) {
	...
	//	var nameHTML = $("<h3>" + amount+ " " + this.per + " " +  this.name + "</h3>");
		var nameHTML = $("<h3>" + amount+ " " + this.per + " " +  this.name + "</h3><span>($" + this.cost + " per " + this.per + ")</span>");
		//var costHTML = $("<span>($" + this.cost + " per " + this.per + ")</span>");
		var supplierHTML = $("<div></div>");
		this.supplier.render(supplierHTML);
		var finalHTML = $("<div class='col-xs-1 col-sm-1 table-bordered img-rounded' style='background-color:lavender;'></div>");
...
a1.Recipe = function(name, ingredients ) {
	...
	this.render = function(base) {
		...
		var finalHTML = $("<br><br><br><br><br><br><br><br><br><br><br><div class='recipe'></div>");
		finalHTML.append(inputHTML);
		finalHTML.append(labelHTML);
		finalHTML.append(nameHTML);
		finalHTML.append(ingredientsHTML);
		base.append(finalHTML);
		finalHTML.hide().delay(1000).slideDown(1000);
		};
...
a1.Supplier = function(name) {
	this.name = name;
	this.render = function(base) {
		var nameHTML = $("<h4 class='text-primary'>" + this.name+ "</h4>");
		var finalHTML = $("<div></div>");
		finalHTML.append(nameHTML);
		base.append(finalHTML);
	};
